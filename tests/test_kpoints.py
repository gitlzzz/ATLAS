...
def test_both_kpoint_style_given():
    _ = atl_aut.generate_kpoints_data(
        structure=...,
        calc_type=...,
        kspacing=0.1,
        kspacing_vec=[1, 1, 1],
...
def test_kpoints_surface():
    ...
    calc_type = atl_aut.CalcType.single_point_surface
    ...
    for miller in list(it.product(list(range(3 + 1)), repeat=3))[1:]:
        ...
        for slab in slabs:
            slab = atl_surf.slab_to_bottom(slab)

            kspacing_vec = atl_aut.kpoint_mesh_from_density(slab, kspacing=kspacing)

            if np.all(np.equal(kspacing_vec, 1)):
                kspacing_vec = atl_aut.kpoint_mesh_from_density(
                    slab, 0.0994491889005363
                )

            kpoints_data = atl_aut.generate_kpoints_data(
                structure=slab,
                calc_type=calc_type,
                kspacing=kspacing,
            )
