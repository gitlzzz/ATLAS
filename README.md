...
The active learning protocol is defined as follows:
...
flowchart TD
 subgraph Initialization["Initialization"]
        Start(["Start AL"])
        Din[("Initial database<br>D<sub>in</sub>")]
        Dt[("Training database<br>D<sub>T</sub>")]
        Ds[("Seed generation<br>database D<sub>S</sub>")]
  end
 subgraph subGraph1["Model Training"]
        Train("Train models with all data")
        M0("Model for<br>trajectory generation<br>M<sub>0</sub>")
        Committee("Committee models<br>M<sub>1</sub>, M<sub>2</sub>, M<sub>3</sub>, M<sub>4</sub>")
  end
 subgraph subGraph2["MD Seed Selection"]
    direction LR
        StopAL2(["Stop AL"])
        AllRemoved{"All structures removed?"}
        Select("Select structure subset (10\%)")
  end
 subgraph subGraph3["Data Aquisition"]
    direction LR
        RunMD("Run MD for each S<sub>i</sub><br>using M<sub>0</sub>")
        ComputeEF("Compute E&amp;F for all frames<br>using committee models")
  end
 subgraph subGraph4["Uncertainty Quantification"]
        ChooseTraj("Choose a trajectory<br>generated by a structure S<sub>i</sub>")
        Check("Check chem. acc. & extrapolation")
        PredAgree{"Do predictions<br>from the models agree?"}
        WellRep("Structure well represented by the models")
        NotWellRep("Structure not well represented<br>by the models")
        RemoveSi("Remove Si from training seed DB")
        StructRemain{"Structures<br>remaining?"}
        GetDFT("Obtain DFT<br>Energies")
        AddData("Add DFT data to D<sub>t</sub>")
  end
 subgraph subGraph5["Safeguard & Termination"]
        LongMD("Long MD<br>safeguard")
        DisagreeAny{"Disagreement<br>on any structure?"}
        FinalDisagree{"Disagreement?"}
        StopAL(["Stop AL"])
  end
    Start --> Din
    Din --> Dt & Ds
    Dt -.-> Train
    Ds -.-> AllRemoved
    AllRemoved -- YES --> StopAL2
    AllRemoved -- NO --> Select
    Train --> M0 & Committee
    M0 --> RunMD
    Committee --> ComputeEF
    Select --> RunMD
    RunMD --> ComputeEF
    ComputeEF --> ChooseTraj
    ChooseTraj --> Check
    Check --> PredAgree
    PredAgree -- YES --> WellRep
    WellRep --> RemoveSi
    RemoveSi --> StructRemain
    StructRemain -- YES --> ChooseTraj
    PredAgree -- NO --> NotWellRep
    NotWellRep --> GetDFT
    GetDFT --> AddData
    StructRemain -- NO --> LongMD
    LongMD --> DisagreeAny
    DisagreeAny -- YES --> NotWellRep
    DisagreeAny -- NO --> FinalDisagree
    FinalDisagree -- NO --> StopAL
     StopAL2:::Rose
     AllRemoved:::Ash
     PredAgree:::Ash
     WellRep:::Aqua
    ...
    style Dt fill:#9ad0bcff
    style Initialization fill:#f6fbf8ff,stroke:#003419ff
    style subGraph2 fill:#faf6fbff
    style subGraph1 fill:#f6f7fbff
    style subGraph3 fill:#fbf9f6ff
    style subGraph4 fill:#fbf6f6ff
    style subGraph5 fill:#f6fbfbff
```
