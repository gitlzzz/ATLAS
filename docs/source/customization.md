...
MatDBForge includes a custom `ImagePNGData` class that wraps AiiDA's `SinglefileData`. The custom image class includes some utility methods to display images automatically, since plots are automatically generated during active learning loops and database generation.
