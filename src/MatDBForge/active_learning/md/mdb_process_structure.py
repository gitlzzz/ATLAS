...
if __name__ == '__main__':
    ...
    mdb_cut.custom_print(f"Using random seed: '{rng_seed}'")

    mdb_cut.custom_print('Starting process structure script...', 'info')
    ...
    for T_start in T_list:
        ...
        print()
        mdb_cut.custom_print(f"Running MD simulation for 'T={T_start} K'", 'info')
    ...
    for curr_traj in traj_files:
        print()
        mdb_cut.custom_print(f"Checking extrapolating frames for '{curr_traj}'", 'info')
        extrap_frame_idx = []
        ...
        mdb_cut.custom_print(f'Trajectory length: {orig_md_size}', 'info')
        ...
        frames_to_remove = []
        mdb_cut.custom_print('Applying MD filters to remove outliers...', 'info')

        if md_filters.get('layer_distance', {}).get('enable'):
            mdb_cut.custom_print("Running 'layer distance' filter...", 'info')
        ...
        if md_filters.get('save_filtered_structures'):
            ...
            mdb_cut.custom_print('Filtered structures not saved.', 'info')
        ...
        mdb_cut.custom_print('Running committee evaluation...', 'info')
        ...
        mdb_cut.custom_print('Printing extrapolation statistics for E:', 'debug')
        ...
        if ef_disagreement_type == 'training':
            ...
            mdb_cut.custom_print(f'e_error_threshold: {e_error_threshold}', 'none')
            mdb_cut.custom_print(f'e_maximum_value: {maximum_value_e}', 'none')
            mdb_cut.custom_print(f'energies_stat: {energies_stat}', 'none')
            ...
            mdb_cut.custom_print('Printing extrapolation statistics for F...', 'none')
            ...
            mdb_cut.custom_print(f'f_error_threshold: {f_error_threshold}', 'none')
            mdb_cut.custom_print(f'f_maximum_value: {maximum_value_f}', 'none')
            mdb_cut.custom_print(f'forces_std_norm_max: {forces_std_norm_max}', 'none')
            ...
            mdb_cut.custom_print(f'e_mean_error: {e_mean_error}', 'none')

            # Spread of variability in the energy predictions across frames
            e_std_error = np.std(energies_stat)
            mdb_cut.custom_print(f'e_std_error: {e_std_error}', 'none')
            ...
            mdb_cut.custom_print(f'e_maximum_value: {e_maximum_value}', 'none')
        ...
        if extrap_type != 'none':
            ...
            mdb_cut.custom_print('Generating descriptors...', 'info')
        ...
        if extrap_type == 'advanced':
            ...
            if dim_red_method == 'pca':
                ...
                if (prepend_path / 'autoencoder_model.pth').exists():
                    ...
                    mdb_cut.custom_print(f"Model loaded from:'{model_path}'")
                else:
    ...
    mdb_cut.custom_print('Structure processed!', 'done')
