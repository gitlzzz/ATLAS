...
def run_initial_config():
    mdb_cut.init_logger(source=pl.Path(__file__).stem, log_path='/tmp')

    # Config file name
    config_file_name = 'secrets.json'

    # Get config directory
    config_path = mdb_cut.get_config_path()

    # Create a mdb folder
    created, config_dir = mdb_cut.init_config_dir(
    ...
    # Get cache directory
    cache_path = mdb_cut.get_cache_path()

    # Create an mdb folder inside the ~/.cache directory
    cache_dir = mdb_cut.init_cache_dir(cache_path)
    ...
    if cache_dir:
        ...
        mdb_cut.custom_print('Cache directory already exists. Nothing done.', 'done')
