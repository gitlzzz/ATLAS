"""Compute diversity/coverage metrics from MatDBForge database extxyz files."""
...
from MatDBForge.core.database import diversity_metrics as mdb_div
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Compute metrics from matdbforge extxyz files.'
    )
    parser.add_argument(
        'datasets',
        nargs='+',
...
def compute_vendi_score(
    ...
    for step, uuids in step_to_uuid_mapping.items():
        ...
        vendi_score = mdb_div.get_vendi_score_subsampling(
...
def process_data_boundaries(uuid_mapping, latent_space_dict, selected_steps):
    ...
    for step, uuids in uuid_mapping.items():
        ...
        results = mdb_morph.process_morphological_closing(
...
def associate_steps_to_uuids(database, latent_space_dict):
    last_step = max([struct.info.get('mdb_al_step', 0) for struct in database])
    step_to_uuid = {step: [] for step in range(last_step + 1)}
    for struct in database:
        step = struct.info.get('mdb_al_step', 0)
        uuid = struct.info.get('mdb_id')
...
def run_database_analysis():
    ...
    for dataset_path_str in args.datasets:
        ...
        if args.selected_steps.strip().lower() == 'all':
            all_steps = sorted(
                {struct.info.get('mdb_al_step', 0) for struct in curr_db}
            )
        ...
        descr_dict, descr_arr, uuids = mdb_al_ut.generate_descriptors(
            database=curr_db,
        ...
        # Get latent space
        latent_space_dict = mdb_ae.get_latent_space_autoencoder(
