...
def console_output(url: str, port: str, dash_pid: str, process_pk: str, log_path: str):
    ...
    store_output_file = f'atl_dashboard_run_{process_pk}_info.out'
    ...
    table.add_row('Dashboard log file', '[dim].atl_dashboard_output.log')
    table.add_row('PID File', '[dim].atl_dashboard.pid')
    ...
    sb.call(
        [
            ...
            *debug,
            '--capture-output',
            '--error-logfile',
            './.atl_dashboard_output.log',
            '--pid',
            './.atl_dashboard.pid',
            '--name',
            'atl_dashboard',
            '--timeout=180',
            f"atlas.active_learning.dashboard.training_dashboard_flask:run_training_dashboard(workchain_node_id='{process_id}',refresh_interval='{update_interval}',port='{port}')",
        ]
    )

    # Wait for the dashboard to start and get the pid
    time.sleep(5)
    with open('./.atl_dashboard.pid') as f:
...
def monitor_al_loop():
    parser = argparse.ArgumentParser(
        ...
        description='Monitor a ATL active learning loop.',
