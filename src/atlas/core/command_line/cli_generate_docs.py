...
""")
    ...
    lines.append(
        ...
        + f'`atl_gen_configuration_file -t {command}`.'
    )
    lines.append('')
    lines.append(':::{attention}')
...
def generate_docs():
    ...
    parser = argparse.ArgumentParser(
        prog='atl_gen_docs',
        description='Generate ATL documentation from the YAML schema.',
    ...
    schema_path = pl.Path(ATL_DATA_DIR) / 'config_schema.yaml'
    try:
