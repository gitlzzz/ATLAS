...
import atlas.core.utils as atl_ut
...
def process_row_parallel(
    ...
    for structure_obj, supr_idx in zip(
        ...
        structure = atl_ut.create_symmetrical_prototype(
            structure=structure,
        ...
        n_at_replacement_upd = atl_ut.fit_replacements_phase(
        ...
        for str_ind, n_atoms in enumerate(n_at_replacement_upd):
            for repl in range(num_repeat_replace):
                ...
                new_structure = atl_ut.apply_replacement(
                    structure=structure,
                    phase=phase,
                    n_target_at=n_atoms,
                    phase_diagram=phase_diagram,
...
def gen_surfaces_diff_miller_parallel(
    ...
    if isinstance(phase, str):
        phase = atl_indb.CuZnInitialDatabase.DB_PHASE_DIAGRAM.get_phase(phase)
