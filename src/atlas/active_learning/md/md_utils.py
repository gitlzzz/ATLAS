...
class CoexistenceMDTemperatureLogger(IOContext):
    ...
    def __init__(
        self,
        # not fully annotated so far to avoid a circular import
        dyn: Any,
        atoms: Atoms,
        logfile: IO | str,
        header: bool = True,
        ...
        if self.dyn is not None:
            self.hdr = '%-9s ' % ('Time[ps]',)  # noqa: UP031
            self.fmt = '%-10.4f '
        else:
            self.hdr = ''
            self.fmt = ''
        if self.peratom:
            self.hdr += '%12s %12s %12s  %6s' % (  # noqa: UP031
                'Etot/N[eV]',
                'Epot/N[eV]',
                'Ekin/N[eV]',
                'T[K]',
            ...
            if global_natoms <= 100:
                ...
                digits = 1
            self.fmt += 3 * ('%%12.%df ' % (digits,)) + ' %6.1f'  # noqa: UP031
        if self.stress:
            self.hdr += (
                '      ---------------------- stress [GPa] -----------------------'
            )
