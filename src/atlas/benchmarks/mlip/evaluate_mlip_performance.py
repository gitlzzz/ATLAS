...
import atlas.benchmarks.mlip.mlip_benchmarks as atl_benchmarks
...
def main():
    ...
    parsed_args = atl_b_ut.parse_arguments()
    init_logger(source='atl_benchmark_mlip')

    # Determine config path
    config_path = parsed_args.config_path
    if config_path is None:
        default_config = pl.Path('./atl_benchmark_settings.toml')
        if default_config.exists():
            config_path = default_config
            custom_print(f'Found default config file: {default_config}', 'info')
        else:
            custom_print(
                'No configuration file specified and atl_benchmark_settings.toml '
                'not found in current directory.',
                'error',
            )
            custom_print(
                'Please provide a configuration file or create '
                'atl_benchmark_settings.toml',
                'error',
            )
            return
    ...
    toml_dict = atl_b_ut.load_toml_config(config_path)
    args = atl_b_ut.create_args_from_toml(toml_dict)
    ...
    if args.run_accuracy_test_set:
        ...
        benchmark_functions['Accuracy Test Set'] = (
            lambda: atl_benchmarks.run_accuracy_test_set_benchmark(args, model_paths)
        )

    if args.run_elastic_properties:
        benchmarks_to_run.append('Elastic Properties')
        benchmark_functions['Elastic Properties'] = (
            lambda: atl_benchmarks.run_elastic_properties_benchmark(args, model_paths)
        )
    ...
    if args.run_defect_formation_energy:
        ...
        benchmark_functions['Defect Formation Energy'] = (
            lambda: atl_benchmarks.run_defect_formation_energy_benchmark(
                args, model_paths
            )
        )
    ...
    if args.run_learning_curves:
        ...
        benchmark_functions['Learning Curves'] = (
            lambda: atl_benchmarks.run_learning_curves_benchmark(args, model_paths)
        )

    if args.run_final_db_size:
        benchmarks_to_run.append('Final DB Size')
        benchmark_functions['Final DB Size'] = (
            lambda: atl_benchmarks.run_final_db_size_benchmark(args, model_paths)
        )

    if args.run_md_count:
        benchmarks_to_run.append('MD Count')
        benchmark_functions['MD Count'] = lambda: atl_benchmarks.run_md_count_benchmark(
            args, model_paths
        )
    ...
    if args.run_evaluate_database:
        ...
        benchmark_functions['Evaluate Database'] = (
            lambda: atl_benchmarks.run_evaluate_database(args, model_paths)
        )

    if args.run_magic_cluster:
        benchmarks_to_run.append('Magic Cluster')
        benchmark_functions['Magic Cluster'] = (
            lambda: atl_benchmarks.run_magic_cluster_benchmark(args, model_paths)
        )
    ...
    if args.foundation_models:
        ...
        foundation_model_paths = atl_b_ut.create_foundation_model_paths(
            args.foundation_models
        )
        for foundation_path in foundation_model_paths:
            atl_b_ut.custom_print(f'  - {foundation_path.name}', 'info')
    ...
    for model_path in model_paths:
        if hasattr(model_path, 'foundation_model_spec'):
            # Foundation model
            atl_b_ut.set_model_display_name(str(model_path), model_path.name)
        else:
            # File-based model
            atl_b_ut.set_model_display_name(str(model_path), model_path.stem)

    if not args.no_rich_ui:
        # Use Rich UI
        ui_manager = atl_b_ut.RichUIManager(benchmarks_to_run)
        atl_b_ut.set_ui_manager(ui_manager)

        with ui_manager:
            atl_b_ut.custom_print('Initializing MLIP Benchmark Suite...', 'info')
            atl_b_ut.custom_print(f'Output directory: {args.output_dir}', 'empty')
            atl_b_ut.custom_print(
            ...
            # Load models
            if args.aiida_pks:
                ui_manager.log('Loading models from ATL Workchain...')
                for pk in args.aiida_pks:
                    path = atl_b_ut.load_model_from_aiida(pk, args.output_dir)
                    if path:
            ...
            atl_b_ut.initialize_model_data(model_paths)
            atl_b_ut.custom_print(f'Initialized {len(model_paths)} model(s).', 'info')
            ...
            atl_b_ut.custom_print('All selected benchmarks finished!', 'done')

            # Create final multi-panel plot if any data was collected
            ui_manager.current_benchmark = 'Generating Summary Plot'
            atl_b_ut.create_final_multi_panel_plot(args)

            time.sleep(2)
    else:
        # Use plain text output (original behavior)
        atl_b_ut.custom_print('Initializing MLIP Benchmark Suite...', 'info')
        atl_b_ut.custom_print(f'Output directory: {args.output_dir}', 'info')
        atl_b_ut.custom_print(
        ...
        # Load models
        if args.aiida_pks:
            atl_b_ut.custom_print('Loading models from ATL Workchain...', 'info')
            for pk in args.aiida_pks:
                path = atl_b_ut.load_model_from_aiida(pk, args.output_dir)
                if path:
                    model_paths.append(path)
        ...
        if not model_paths:
            atl_b_ut.custom_print('No models specified. Exiting.', 'error')
            return

        atl_b_ut.custom_print(f'Loaded {len(model_paths)} model(s)', 'info')
        for path in model_paths:
            display_name = atl_b_ut.get_model_display_name(path)
            atl_b_ut.custom_print(f'  - {display_name}', 'info')
        ...
        for benchmark_name in benchmarks_to_run:
            try:
                atl_b_ut.custom_print(f'Starting benchmark: {benchmark_name}', 'info')
                benchmark_functions[benchmark_name]()
                atl_b_ut.custom_print(f'Completed benchmark: {benchmark_name}', 'done')
            except Exception as e:
                raise e
                atl_b_ut.custom_print(
