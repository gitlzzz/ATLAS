...
import atlas.benchmarks.mlip.mlip_benchmark_utils as atl_b_ut
from atlas.active_learning.report_utils import (
    get_loop_report,
)
...
def run_final_db_size_benchmark(args, model_paths: list[pl.Path]):
    ...
    atl_b_ut.custom_print('Running Final Database Size Benchmark', 'info')
    ...
    for pk in args.aiida_pks:
        try:
            ...
            atl_b_ut.custom_print(f'Processing workchain {pk} ({run_name})...', 'info')

            # Get all children workchains to find the final one
            all_children = base_workchain.called
            if not all_children:
                atl_b_ut.custom_print(f'No children found for workchain {pk}', 'warn')
                continue
            ...
            atl_b_ut.custom_print(f'Failed to process workchain {pk}: {e}', 'error')
            continue
    ...
    if results:
        ...
        # Print summary
        atl_b_ut.custom_print('Database Size Summary:', 'info')
        ...
        atl_b_ut.custom_print(f'Results saved to {results_file}', 'info')
    else:
        atl_b_ut.custom_print('No database size results to save.', 'warn')
...
def run_md_count_benchmark(args, model_paths: list[pl.Path]):
    ...
    atl_b_ut.custom_print('Running MD Count Benchmark', 'info')
    ...
    for model_path in model_paths:
        ...
        model_name = atl_b_ut.get_model_display_name(model_path)
    ...
    if results:
        ...
        # Print summary
        atl_b_ut.custom_print('MD Count Summary:', 'info')
        for name in run_names:
            data = results[name]
            if data['pk']:
                atl_b_ut.custom_print(
        ...
        atl_b_ut.custom_print(f'Results saved to {all_results_file}', 'info')
    else:
        atl_b_ut.custom_print('No MD count results to save.', 'warn')


def run_melting_point_benchmark(args, model_paths: list[pl.Path]):
    atl_b_ut.custom_print('Running Melting Point Benchmark', 'info')
    ...
    for _, model_path in enumerate(model_paths):
        ...
        try:
            ...
            if 'lammps.pt' not in model_path.name:
                calculator = atl_b_ut.create_calculator_for_model(
    ...
    if results:
        ...
        # Print summary
        atl_b_ut.custom_print('Melting Point Summary:', 'info')
        for name, tm in zip(model_names, melting_points, strict=True):
            if tm > 0:
                atl_b_ut.custom_print(f'  {name}: {tm:.1f} K', 'info')
            else:
                atl_b_ut.custom_print(f'  {name}: Unable to determine', 'warning')

        atl_b_ut.custom_print(f'Results saved to {all_results_file}', 'info')
    else:
        atl_b_ut.custom_print('No melting point results to save.', 'warn')
...
def run_energy_md_benchmark(args, model_paths: list[pl.Path]):
    ...
    for _, model_path in enumerate(model_paths):
        model_name = atl_b_ut.get_model_display_name(model_path)
        atl_b_ut.custom_print(f'Running MD for model: {model_name}', 'info')
        ...
        try:
            ...
            atl_b_ut.custom_print(f'MD for {model_name} finished!', 'done')
            ...
            atl_b_ut.custom_print(f'MD for {model_name} failed: {e}', 'error')
    ...
    atl_b_ut.custom_print('Energy MD data collected for final plot', 'info')
...
def run_elastic_properties_benchmark(args, model_paths: list[pl.Path]):
    ...
    atl_b_ut.custom_print('Running Elastic Properties Benchmark', 'info')
    ...
    atl_b_ut.custom_print('Elastic properties benchmark not implemented yet.', 'warn')
...
def run_defect_formation_energy_benchmark(args, model_paths: list[pl.Path]):
    ...
    atl_b_ut.custom_print('Running Defect Formation Energy Benchmark', 'info')
    ...
    if results:
        ...
        atl_b_ut.set_plot_data('defect_formation', plot_data)
        ...
        for group_name in groups:
            ...
            for name, energy in zip(valid_models, formation_energies, strict=False):
                atl_b_ut.custom_print(f'  {name}: {energy:.3f} eV', 'empty')
    else:
        atl_b_ut.custom_print(
...
def run_surface_energies_benchmark(args, model_paths: list[pl.Path]):
    ...
    """
    atl_b_ut.custom_print('Running Surface Energies Benchmark', 'info')
    ...
    if bulk_structure_arg or slab_structures_arg or dft_refs_arg:
        ...
        if bulk_structure_arg:
            atl_b_ut.custom_print(f'  DFT bulk structure: {bulk_structure_arg}', 'info')
        if slab_structures_arg:
            atl_b_ut.custom_print(
                f'  DFT slab structures: {len(slab_structures_arg)} provided', 'info'
            )
        if dft_refs_arg:
            atl_b_ut.custom_print(f'  DFT reference energies: {dft_refs_arg}', 'info')
    ...
    if bulk_structure_arg:
        ...
        struct_format = atl_b_ut._get_structure_format(bulk_structure_path)
    ...
    if dft_refs_arg and pl.Path(dft_refs_arg).exists():
        try:
            ...
            atl_b_ut.custom_print(f'Error loading DFT reference file: {e}', 'error')
    else:
        atl_b_ut.custom_print(
    ...
    for _, model_path in enumerate(model_paths):
        ...
        try:
            ...
            struct_format = atl_b_ut._get_structure_format(bulk_primitive_path)
            ...
            for surface_indices in surfaces_to_test:
                ...
                if surface_name in user_provided_slabs:
                    ...
                    struct_format = atl_b_ut._get_structure_format(slab_path)
                    ...
                    struct_format = atl_b_ut._get_structure_format(bulk_relaxed_path)
    ...
    if results:
        ...
        # Print summary
        atl_b_ut.custom_print('Surface Energy Summary:', 'info')
        for model_name in model_names:
            atl_b_ut.custom_print(f'  {model_name}:', 'empty')
...
def run_phonon_dispersion_benchmark(args, model_paths: list[pl.Path]):
    ...
    atl_b_ut.custom_print('Running Phonon Dispersion Benchmark', 'info')
    ...
    atl_b_ut.custom_print('Phonon dispersion benchmark not implemented yet.', 'warn')
...
def coexistence_structure_melting_point(
    ...
    if not solid_path.exists():
        atl_b_ut.custom_print('Phase 1: Equilibrating solid phase...', 'info')
        ...
        solid_npt.attach(
            atl_al_ut.manual_progress_display,
            interval=1000,
            dyn=solid_npt,
        )
        ...
        atl_b_ut.custom_print(f'Equilibrated solid saved to {solid_path}', 'done')
    else:
    ...
    atl_b_ut.custom_print('Phase 2: Creating and melting liquid phase...', 'info')
    ...
    atl_b_ut.custom_print('Phase 3: Cooling down liquid phase...', 'info')
    ...
    if not coexistence_relaxed_path.exists():
        ...
        if not cooled_liquid_path.exists():
            ...
            atl_b_ut.custom_print('Heating solid to liquid temperature...', 'info')
            ...
            liquid_nvt_heating_ramp.attach(
                interval=1,
                function=atl_al_ut.md_apply_temperature_ramp,
                dyn=liquid_nvt_heating_ramp,
        ...
        atl_b_ut.custom_print('Combined solid and liquid phases into one cell.', 'info')
    ...
    atl_b_ut.custom_print('Phase 4: Coexistence of liquid and solid (NVE)...', 'info')
...
def prepare_coexistence_structure_lammps(
    ...
    atl_b_ut.custom_print('Preparing coexistence structure via LAMMPS...', 'info')

    # --- Phase 1: Prepare equilibrated solid phase ---
    solid_path = benchmark_dir / 'equilibrated_solid.xyz'
    if not solid_path.exists():
        atl_b_ut.custom_print('Phase 1: Equilibrating solid phase (NPT)...', 'info')
        solid_bulk = bulk(metal, cubic=True)
        solid_supercell = solid_bulk.repeat(supercell_size)
        MaxwellBoltzmannDistribution(solid_supercell, temperature_K=solid_temp_K)
    ...
    if not cooled_liquid_path.exists():
        # --- 2a: Melt the solid ---
        atl_b_ut.custom_print('Phase 2a: Melting the structure (NPT)...', 'info')
        liquid_supercell = ase_read(solid_path, format='extxyz')
        MaxwellBoltzmannDistribution(liquid_supercell, temperature_K=liquid_temp_K)
        ...
        # --- 2b: Cool the liquid ---
        atl_b_ut.custom_print('Phase 2b: Cooling the liquid (NPT)...', 'info')
    ...
    atl_b_ut.custom_print('Phase 3: Assembling coexistence structure...', 'info')
    coexistence_atoms = solid_supercell.copy()
    ...
    # --- Phase 4: Coexistence of liquid and solid phases with NVE ---
    atl_b_ut.custom_print('Phase 4: Coexistence interface relaxation (NVE)...', 'info')
    n_steps_nve = 33334
    timestep_nve_fs = 3.0
    atl_b_ut.custom_print(
        f'Relaxing interface (NVE) for '
...
def run_gsfe_benchmark(args, model_paths: list[pl.Path]):
    ...
    atl_b_ut.custom_print('Running GSFE Benchmark', 'info')
    ...
    atl_b_ut.custom_print('GSFE benchmark not implemented yet.', 'warn')
...
def run_learning_curves_benchmark(args, model_paths: list[pl.Path]):
    ...
    atl_b_ut.custom_print('Running Learning Curves Benchmark', 'info')
    ...
    if learning_curves_data:
        ...
        # Print summary
        atl_b_ut.custom_print('Learning Curves Summary:', 'info')
        for run_name, data in learning_curves_data.items():
        ...
        atl_b_ut.custom_print('No learning curve results to save.', 'warn')
...
def run_evaluate_database(args, model_paths: list[pl.Path]):
    ...
    """
    atl_b_ut.custom_print('Running Database Evaluation Benchmark', 'info')
    ...
    if not args.database_path.exists():
        atl_b_ut.custom_print(f'Database file not found: {args.database_path}', 'error')
        return
    ...
    atl_b_ut.custom_print(f'Loading database from: {args.database_path}', 'info')
    try:
        structures = ase_read(args.database_path, index=':')
        atl_b_ut.custom_print(f'Loaded {len(structures)} structures', 'info')
    except Exception as e:
        atl_b_ut.custom_print(f'Failed to load database: {e}', 'error')
        return
    ...
    for _, model_path in enumerate(model_paths):
        model_name = atl_b_ut.get_model_display_name(model_path)
        atl_b_ut.custom_print(f'Evaluating model: {model_name}', 'info')
    ...
    if results:
        ...
        # Generate plots
        atl_b_ut.custom_print('Generating evaluation plots...', 'info')

        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10))

        # Color map for different models
        model_names = list(results.keys())
        colors = atl_b_ut.get_model_colors_by_names(model_names)
        ...
        png_path, svg_path = atl_b_ut.save_plot_dual_format(base_path, dpi=300)

        atl_b_ut.custom_print(f'Plot saved to {png_path}', 'done')
        atl_b_ut.custom_print(f'Plot saved to {svg_path}', 'done')
        plt.close()
        ...
        # Print summary
        atl_b_ut.custom_print('Database Evaluation Summary:', 'info')
        for name in model_names:
            data = results[name]
            atl_b_ut.custom_print(
        ...
        atl_b_ut.custom_print('No database evaluation results to save.', 'warn')
...
def run_magic_cluster_benchmark(args, model_paths: list[pl.Path]):
    ...
    """
    atl_b_ut.custom_print('Running Magic Cluster Benchmark', 'info')
    ...
    if results:
        ...
        atl_b_ut.custom_print(f'Results loaded for {len(results)} models:', 'debug')
        ...
        atl_b_ut.custom_print('Cluster energies data for plotting:', 'debug')
        ...
        # Print summary
        atl_b_ut.custom_print('Magic Cluster Energy Summary:', 'info')
        for n_atoms in magic_numbers:
            atl_b_ut.custom_print(f'  Cluster size {n_atoms} atoms:', 'empty')
            for model_name in model_names:
        ...
        atl_b_ut.custom_print('No magic cluster results to save.', 'warn')
